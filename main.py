...
from news_finbert import news_results
...
bert_score = news_results('nvidia')
print("\nFinBERT score:", bert_score)

prices = get_stock_data()
print(f"\nToday's opening price for {symbol}: {prices['today_open']}")

writer_object.writerow([datetime.date.today(), symbol, vader_score, bert_score, 'NA', prices['yesterday_close'], prices['today_open']])
